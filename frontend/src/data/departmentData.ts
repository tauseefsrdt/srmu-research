...
export const DEPARTMENTS_LIST = [
  ...
  },
  {
    ...
    programs: [
      "Education & Research",
      "Ph.D in Advance Educational Studies",
...
export const getAllDepartmentsInfo = (): DepartmentInfo => {
  ...
  const totalDesignationLimit = VACANT_SEAT_DATA.reduce(
    (acc, r) => acc + (r.designationSeatLimit || 0),
    0
  );
  const totalAllottedSeats = VACANT_SEAT_DATA.reduce(
    (acc, r) => acc + (r.allottedSeat || 0),
    0
  );
  const totalVacantSeats = VACANT_SEAT_DATA.reduce(
    (acc, r) => acc + (r.noOfVacant || 0),
    0
  );
...
export const getDepartmentById = (idOrSlug: string): DepartmentInfo | null => {
  ...
  const config = DEPARTMENTS_LIST.find(
    (d) => d.id === clean || d.slug === clean || d.code.toLowerCase() === clean
  );
  if (!config) return null;

  // Filter exact faculty supervisors from Vacant Seat data
  const facultySupervisors = VACANT_SEAT_DATA.filter(config.vacantMatcher);
  ...
  const totalDesignationLimit = facultySupervisors.reduce(
    (acc, r) => acc + (r.designationSeatLimit || 0),
    0
  );
  const totalAllottedSeats = facultySupervisors.reduce(
    (acc, r) => acc + (r.allottedSeat || 0),
    0
  );
  const totalVacantSeats = facultySupervisors.reduce(
    (acc, r) => acc + (r.noOfVacant || 0),
    0
  );
